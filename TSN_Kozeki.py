...
Log.Clear();
Kozeki_Version: str = "v0.2.0";
Kozeki_Branch: str = "Azure";



def Extract_Slow(F: str, Path: str) -> None:
	...
	File.Path_Require(f"Extracted/{Path}");
	File.Path_Require(f"Unknown/{Path}");
	...
	for Index, Byte_Code in enumerate(Bytes):
		...
		if (OxFF and JPEG_End):
			...
			with open(f"Extracted/{Path}/{Index}.jpg", "w+b") as JPEG: JPEG.write(Buffer_JPEG); Buffer_JPEG = b"";
			...
			if (len(Buffer_Unknown) > 1):
				Buffer_Unknown[:-1];
				with open(f"Unknown/{Path}/{Index}.hex", "w+b") as Hex: Hex.write(Buffer_Unknown);
...
def Extract_Deep(F: str, Path: str) -> None:
	...
	while (Bytes.find(b"\xFF\xD8") != -1):
		...
		if (JPEG_Start != 0):
			with open(f"Unknown/{Path}/0x{Offset}-0x{Offset + JPEG_Start}.hex", "w+b") as Hex: Hex.write(Bytes[:JPEG_Start]);
			Log.Info("Unknown data found before JPEG File, writing.");

		with open(f"Extracted/{Path}/0x{Offset + JPEG_Start}-0x{Offset + JPEG_End}.jpg", "w+b") as Hex: Hex.write(Bytes[JPEG_Start:JPEG_End]);
			...
			for File in Files:
				if (File.endswith(".molru")):
					...
					match Extractor.lower():
						case "slow": Extract_Slow(f"{Path}{File}", Path);
						case "deep": Extract_Deep(f"{Path}{File}", Path);
